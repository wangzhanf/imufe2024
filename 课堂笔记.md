...
要求创建自己的工具箱mytools，  通过工具箱快速调用自己创建的工具，如  查看文件状态的  mystat(仿stat)   ，   查看目录列表的  myls （仿ls -lia）  ，    复制文件的mycp（仿cp），  查看当前系统内存信息   mymi（读取 /proc/meminfo），   使用方式要求为如下形式
...
# mytools mymi
 ...
 提交详细的实现和验证过程  以及  结论    ，    附属   小组组内评分   ，提交到    github仓库
